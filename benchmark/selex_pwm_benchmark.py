...
REF_PATH = BASE_PATH + '/greco-assembly/hg19.fa'
PWM_PATH = BASE_PATH + \
    '/greco-motifs/motifs_organized_v5_pwm/{name}/{name}@{pwm_name}.pwm'
THR_PATH = BASE_PATH + \
    '/greco-motifs/motifs_organized_thr/{name}@{pwm_name}.thr'
FASTA_PATH = BASE_PATH + \
    '/greco-fasta/motifs_organized_fasta/{name}@{pwm_name}{prefix}.fasta'
PV_PATH = BASE_PATH + \
    '/greco-fasta/motifs_organized_fasta/' + \
    '{name}@{pwm_name}{prefix}.processed.fasta'
BATCH1_PATH = BASE_PATH + '/selex/batch1/'  # '/selex/GVATdb.csv'
BATCH2_PATH = BASE_PATH + '/selex/batch2/'
# '/selex/GVATdb.novel_batch.csv'

TP_THR = 0.01  # True positive threshold
TN_THR = 0.5  # True negative threshold
LH_THR = 0.05  # Likelihood threshold for 1st batch
...
def get_pvalues(fasta_path, pwm_path, thr_path):
    return subprocess.check_output(['java', '-cp', './sarus-2.0.2.jar',
                                    'ru.autosome.SARUS',
                                    fasta_path, pwm_path,
                                    'besthit',
...
if __name__ == '__main__':
    with open('selex_motifs_organized_v5.tsv', 'a') as results:
